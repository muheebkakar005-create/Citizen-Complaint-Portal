 ...
 *          with a guaranteed local fallback. Used by the officer/admin
...
const getComplaintSummary = asyncHandler(async (req, res) => {
  ...
  const complaintWithPriority = withPriority(complaint);
  const { summary, source } = await generateComplaintSummary(complaintWithPriority);

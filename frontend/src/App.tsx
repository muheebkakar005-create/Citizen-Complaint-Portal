...
<<<<<<< HEAD
      ...
      if (user.role !== 'admin') {
        return (
          <div className="max-w-md mx-auto my-16 p-8 bg-white rounded-2xl border border-slate-200 text-center space-y-4 shadow">
            ...
            <h2 className="text-lg font-bold text-slate-900">Access Restricted</h2>
            <p className="text-xs text-slate-600">
              Administrator privileges are required to view this console.
            </p>
            <button
              onClick={() => navigate('/dashboard')}
              className="px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-semibold"
    ...
    // 8c. Admin Review Single Complaint (/admin/complaints/:id)
    if (currentPath.startsWith('/admin/complaints/')) {
      const complaintId = currentPath.replace('/admin/complaints/', '').trim();
      if (!user) {
        return <LoginPage navigate={navigate} />;
      }
      if (user.role !== 'admin') {
        return (
          <div className="max-w-md mx-auto my-16 p-8 bg-white rounded-2xl border border-slate-200 text-center space-y-4 shadow">
            <ShieldAlert size={32} className="mx-auto text-rose-600" />
            <h2 className="text-lg font-bold text-slate-900">Administrator Authorization Required</h2>
            <button
              onClick={() => navigate('/complaints')}
              className="px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-semibold"
            >

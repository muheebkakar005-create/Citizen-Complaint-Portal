  ...
  return (
    <div className="w-full space-y-10 py-6 font-sans">
      ...
      <motion.section
        ...
        className="relative overflow-hidden rounded-2xl text-brand-dark p-4 sm:p-8 lg:p-14 shadow-xl border border-brand-gold/30 bg-brand-cream/80 min-h-[320px] sm:min-h-[420px] lg:min-h-[520px] flex items-center"
      >
        {/* Background */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center relative z-10 w-full">
          <motion.div
            ...
            className="lg:col-span-7 space-y-3 sm:space-y-5 bg-white/80 sm:bg-white/70 md:bg-white/80 lg:bg-transparent backdrop-blur-xs md:backdrop-blur-none p-4 sm:p-6 lg:p-0 rounded-xl lg:rounded-none border border-white/60 lg:border-0 shadow-sm lg:shadow-none"
          >
            {/* Tagline strip */}
            <div className="inline-flex items-center gap-2 text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] text-brand-green">
              <span className="w-5 h-[2px] bg-brand-gold"></span>
              <span>A Cleaner &bull; Safer &bull; Stronger Balochistan</span>
            </div>

            <div className="space-y-1">
              <h1 className="text-2xl sm:text-4xl lg:text-6xl font-extrabold tracking-tight leading-[1.1]">
                <span className="text-brand-dark">Citizen Complaint</span>
                <br />
                <span className="text-brand-green">Portal</span>
              </h1>
              <p className="text-base sm:text-2xl font-bold text-brand-dark pt-0.5">Your Voice Matters</p>
              <p className="text-[10px] sm:text-sm font-bold uppercase tracking-[0.2em] text-brand-gold">
                Report &bull; Track &bull; Resolve
              </p>
            </div>

            <p className="text-xs sm:text-base text-slate-700 leading-relaxed max-w-xl font-medium">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-1">
              <motion.button
                ...
                className="px-4 sm:px-6 py-2.5 sm:py-3 rounded-full bg-brand-green-deep hover:bg-brand-green text-white font-bold text-xs sm:text-sm uppercase tracking-wider shadow-md flex items-center gap-1.5 transition-all cursor-pointer"
              >
                <FilePlus2 size={14} />
              ...
              <motion.button
                ...
                className="px-4 sm:px-6 py-2.5 sm:py-3 rounded-full bg-white hover:bg-brand-green/5 text-brand-green font-bold text-xs sm:text-sm uppercase tracking-wider border-2 border-brand-green shadow-sm flex items-center gap-1.5 transition-all cursor-pointer"
              >
                <Search size={14} />
          ...
          {/* Right Column: hidden on small mobile, visible lg+ */}
          <motion.div
            ...
            className="hidden lg:flex lg:col-span-5 w-full flex-col items-center justify-center gap-4"
          >
      ...
      <section className="space-y-3">
        <div>
          <h2 className="text-base sm:text-xl font-extrabold text-brand-dark">Quick Services</h2>
          <p className="text-xs text-slate-500">Fast. Transparent. Citizen Focused.</p>
        </div>
        {/* Horizontal scroll on mobile, grid on sm+ */}
        <div className="flex sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-3 overflow-x-auto pb-2 sm:pb-0 snap-x snap-mandatory">
          {[
            ...
            <motion.div
              ...
              className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-2 cursor-default transition-colors flex items-start gap-3 min-w-[220px] sm:min-w-0 shrink-0 sm:shrink snap-start"
            >
              <div className={`w-9 h-9 shrink-0 rounded-full ${feat.color} text-white flex items-center justify-center shadow-sm`}>
                <feat.icon size={16} />
              </div>
              <div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-base sm:text-lg font-bold text-slate-800">Recent Incidents</h2>
            <p className="text-xs text-slate-400">Live feed across Balochistan</p>
          </div>
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => navigate('/complaints')}
            className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-teal-700 hover:text-brand-dark transition-colors cursor-pointer shrink-0"
          >
            <span>View All</span>
            <ArrowRight size={12} />
          </motion.button>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="h-36 rounded-xl bg-white animate-pulse border border-slate-200" />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            {recentComplaints.map((complaint, idx) => (
              <motion.div
                ...
                whileHover={{ y: -4, borderColor: '#c9a227', boxShadow: '0 10px 25px -6px rgba(11, 61, 44, 0.1)' }}
                whileTap={{ scale: 0.985 }}
                onClick={() => navigate(`/complaints/${complaint._id}`)}
                className="p-4 rounded-xl bg-white/95 border border-slate-200 transition-all cursor-pointer flex flex-col justify-between space-y-2.5 group"
              >
